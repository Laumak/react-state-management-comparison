...
Redux Toolkit is the official, opinionated toolset for Redux. It reduces boilerplate with utilities
like `createSlice`, `createAsyncThunk`, and the `configureStore` helper.
...
Fetching Pokémon data from the [PokéAPI](https://pokeapi.co/) using RTK's `createAsyncThunk` and/or
**RTK Query** for data fetching, caching, and loading/error state management.
...
✅ **Implemented.** Run `pnpm dev --filter @repo/with-redux-toolkit` from the repo root to start the
dev server.
